...
namespace MusicSchool
{
    class StudentFactory : IStudentFactory
    {
        ...
        public Student CreateStudent(int type)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                subjects.Add((Subject)_random.Next(Enum.GetValues(typeof(Subject)).Length));
            }

            Direction direction = (Direction)_random.Next(Enum.GetValues(typeof(Direction)).Length);

            student = new BeginnerStudent(_studentNames[_random.Next(_studentNames.Count)], _random.Next(8, 16),
            ...
            if (type == 0)
            {
                student = new AdvancedStudent(_studentNames[_random.Next(_studentNames.Count)], _random.Next(8, 16),
                    _random.Next(2, 5), _random.Next(1, 10), subjects, direction,
